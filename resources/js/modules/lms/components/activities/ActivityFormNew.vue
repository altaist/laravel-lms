<template>
    <q-card class="full-width">
      ...
      <q-card-section>
        <q-select
          ...
          class="q-mb-md"
          :rules="[val => !!val || 'Выберите группу']"
          @update:model-value="onTeamSelect"
        />
        <q-select
          v-if="scheduleDays.length > 0"
  ...
  const getDefaultStartTime = () => {
    ...
    roundedDate.setMinutes(minutes % 60)
    ...
    const formattedHours = String(roundedDate.getHours()).padStart(2, '0')
  ...
  const onTeamSelect = async (team) => {
    ...
    try {
      const response = await axios.get(route('teams.schedule', team.id))
      ...
      $q.notify({
        type: 'negative',
        message: 'Не удалось загрузить расписание группы',
        position: 'top-right'
      })
    }
  }
  ...
  const onScheduleSelect = (scheduleItem) => {
    const selectedSchedule = scheduleDays.value.find(s => s.id === scheduleItem.id)
    if (selectedSchedule) {
      const now = new Date()
